...
export async function POST(req: NextRequest) {
  const user = await getUser(req)
  if (!user) return NextResponse.json({ ok: false, error: 'Unauthorized' }, { status: 401 })
  const { nickname, email, lang, ai_trade_amount, ai_max_leverage, ai_balance, ai_risk_per_trade } = await req.json()

  // Валидация торговых настроек
  const amount    = ai_trade_amount  != null ? Math.max(10, Math.min(100_000, Number(ai_trade_amount)))  : undefined
  ...
  await updateUserSettings(user.id, { nickname, email, lang, ai_trade_amount: amount, ai_max_leverage: leverage, ai_balance: balance, ai_risk_per_trade: riskPct })
