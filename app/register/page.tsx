      ...
      <div className="auth-wrap">
        <div className="auth-box">
          ...
          <div style={{ marginTop: 16, background: 'rgba(0,230,118,0.08)', border: '1px solid rgba(0,230,118,0.25)', borderRadius: 4, padding: '14px 16px' }}>
            <div style={{ fontSize: '.65rem', color: 'var(--long)', lineHeight: 1.55 }}>
              {t('verify_email_body').replace('{email}', registeredEmail)}
            </div>
          </div>
          <div style={{ marginTop: 16, textAlign: 'center' }}>
            {resendDone ? (
              <span style={{ fontSize: '.63rem', color: 'var(--long)' }}>{t('verify_email_resent')}</span>
            ) : (
              <button
                onClick={handleResend}
                disabled={resendLoading}
                style={{ background: 'none', border: 'none', color: 'var(--cyan)', fontSize: '.63rem', cursor: 'pointer', padding: 0 }}
              >
