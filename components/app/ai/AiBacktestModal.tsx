            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, marginBottom: 14 }}>
                {([
                  ...
                  { label: 'Hit Rate',   value: `${st.hit_rate}%`,     sub: `${st.hit_count} из ${st.total_obs} достигли зоны`,   color: st.hit_rate    >= 60 ? 'var(--long)' : 'var(--wait)' },
                  { label: 'Bounce Rate',value: `${st.bounce_rate}%`,  sub: `отскоков из касаний`, color: st.bounce_rate >= 55 ? 'var(--long)' : st.bounce_rate >= 40 ? 'var(--wait)' : 'var(--short)' },
                  { label: 'Avg R:R',    value: st.avg_rr != null ? `${st.avg_rr}:1` : '—', sub: 'ср. соотношение риск/прибыль', color: (st.avg_rr ?? 0) >= 1.5 ? 'var(--long)' : 'var(--wait)' },
              ...
              <div style={{ fontSize: '.55rem', color: 'var(--dim)', lineHeight: 1.6, borderTop: '1px solid var(--line2)', paddingTop: 8 }}>
                📌 Hit Rate — % OB зон, которых достигла цена в следующих {bd.verify_candles} свечах. Bounce Rate — % отскоков от зоны.
              </div>
            </>
          )
        })()}
