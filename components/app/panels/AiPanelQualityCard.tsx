  ...
  return (
    <div style={{ margin: '8px 0', padding: '9px 12px', background: 'rgba(255,255,255,0.03)', borderRadius: 6, border: `1px solid ${scoreColor}30` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontSize: '.56rem', color: 'var(--dim)', fontWeight: 700, letterSpacing: '.06em' }}>{t('ob_card_title_lbl')}</span>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: '.58rem', color: 'var(--dim)' }}>{t('ob_age_lbl')} <b style={{ color: 'var(--text)' }}>{Number(ob.ageCandles ?? 0)} {t('candles_abbr_lbl')}</b></span>
